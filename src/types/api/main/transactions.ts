...
import { AptosTransaction } from '../shared/txs/aptos.js'
import { SuiTransaction } from '../shared/txs/sui.js'
import { EvmTransaction } from './txs/index.js'

export {
  TransactionType,
  GenericTransactionType, TransactionStatus
}
export type {
  SwapExplorerUrl,
  ReportTransactionRequest, CheckApprovalResponse
}
...
/**
 * List of validations that Mycel should do
 *
 * @property {boolean} balance - If true [Recommended], Mycel will check that user has the required balance for swap
 * @property {boolean} fee - If true [Recommended], Mycel will check that user has the required fees in the wallet
 *
 */
 ...
 * @property {CreateTransactionValidation} validations - the validation checks we are interested to check by Mycel
 ...
 * outside Mycel to get its assets back (refund). You can show this link to the user to help him
 * @property {SwapExplorerUrl[] | null} explorerUrl - List of explorer URLs for the transactions that happened in this step.
 * @property {TransactionStatusReferral[] | null} referrals - List of referral reward for the dApp and Mycel
 ...
 * @property {boolean} ok - If true, Mycel has created a non-null transaction and the error message is null
