import type { EvmBlockchainMeta } from '../../types';
import {
...
export const getAptosAccounts: Connect = async ({ instance }) => {
  // Asking for account from wallet.
  const aptosResponse = await instance.connect();
  return {
    accounts: [aptosResponse.address],
    chainId: Networks.APTOS,
  };
};

export const getSuiAccounts: Connect = async ({ instance }) => {
  // Asking for account from wallet.
  const suiResponse = await instance.connect();
  return {
    accounts: [suiResponse.accounts[0].address],
    chainId: Networks.SUI,
  };
};
