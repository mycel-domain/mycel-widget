...
import { convertEvmChainId } from '../../utils/common';
...
export class DefaultEvmSigner implements GenericSigner<EvmTransaction> {
  ...
  async signTransaction(tx: any): Promise<any> {
    try {
      return await this.signer.signMessage(tx);
    } catch (error) {
      throw new SignerError(SignerErrorCode.SIGN_TX_ERROR, undefined, error);
    }
  }
    ...
    try {
      ...
      if (!!chainId && !!signerChainId && chainId !== signerChainId.toString() ) {
        throw new SignerError(
          ...
          undefined,
          `Signer chainId: '${convertedChainId}' doesn't match with required chainId: '${chainId}' for tx.`
        );
      }
      if (
        !!signerAddress &&
